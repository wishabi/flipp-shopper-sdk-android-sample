...
- ``zoneIds`` - A list of longs  used to separate site/properties within a media partner's portfolio for content targeting. **At lease one zone ID is required.**
For example, a zone for sports, another for news or food.  ZoneIDs are constrained to integer values and must align with the ID's provided by Flipp. Refer to the site configuration in kevel to find the publishers zoneID(s)
- ``contentCode`` - A string for invoking different campaign content for testing purposes. After the testing phase is complete, set this parameter to an empty string.
